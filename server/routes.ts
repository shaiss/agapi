...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/labs", async (req, res) => {
    console.log("[API] POST /api/labs - Creating new lab experiment");
    ...
    try {
      ...
      // Create the lab
      const lab = await storage.createLab(req.user!.id, validatedData);
      
      res.status(201).json(lab);
    } catch (error) {
      console.error("[API] Error creating lab experiment:", error);
      res.status(500).json({ message: "Failed to create lab experiment" });
    }
  });
  
  // Get all labs for the current user
  app.get("/api/labs", async (req, res) => {
    console.log("[API] GET /api/labs - Retrieving all lab experiments");
    ...
    try {
      ...
      console.error("[API] Error retrieving lab experiments:", error);
      res.status(500).json({ message: "Failed to retrieve lab experiments" });
    }
  });
  
  // Get a single lab by ID
  app.get("/api/labs/:id", async (req, res) => {
    console.log(`[API] GET /api/labs/${req.params.id} - Retrieving lab experiment`);
    ...
    try {
      ...
      if (!lab) {
        return res.status(404).json({ message: "Lab experiment not found" });
      }
      ...
      console.error("[API] Error retrieving lab experiment:", error);
      res.status(500).json({ message: "Failed to retrieve lab experiment" });
    }
  });
  
  // Update lab status (launch or complete)
  app.patch("/api/labs/:id/status", async (req, res) => {
    console.log(`[API] PATCH /api/labs/${req.params.id}/status - Updating lab status`);
    ...
    try {
      ...
      // Get the lab first to verify ownership
      const lab = await storage.getLab(parseInt(req.params.id));
      ...
      // Update the status
      const updatedLab = await storage.updateLabStatus(parseInt(req.params.id), status as "draft" | "active" | "completed");
  ...
  // Delete a lab experiment
  app.delete("/api/labs/:id", async (req, res) => {
    console.log(`[API] DELETE /api/labs/${req.params.id} - Deleting lab experiment`);
    ...
    try {
      ...
      // Delete the lab
      await storage.deleteLab(parseInt(req.params.id));
      
      res.sendStatus(204);
    } catch (error) {
      console.error("[API] Error deleting lab experiment:", error);
      res.status(500).json({ message: "Failed to delete lab experiment" });
    }
  });
  
  // Create a lab post
  app.post("/api/labs/:id/posts", async (req, res) => {
    console.log(`[API] POST /api/labs/${req.params.id}/posts - Creating lab post`);
    ...
    try {
      // Validate request body using the schema
      const validatedData = insertLabPostSchema.parse({
        ...req.body,
      ...
      // Verify ownership
      if (lab.userId !== req.user!.id) {
        return res.status(403).json({ message: "Not authorized to add posts to this lab" });
      }
      
      // Create the post
      const post = await storage.createLabPost(validatedData);
      
      res.status(201).json(post);
    } catch (error) {
      console.error("[API] Error creating lab post:", error);
      res.status(500).json({ message: "Failed to create lab post" });
    }
  });
  
  // Get all posts for a lab
  app.get("/api/labs/:id/posts", async (req, res) => {
    console.log(`[API] GET /api/labs/${req.params.id}/posts - Retrieving lab posts`);
    ...
    try {
      ...
      // Verify ownership
      if (lab.userId !== req.user!.id) {
        return res.status(403).json({ message: "Not authorized to view posts for this lab" });
      }
      
      // Get lab posts
      const posts = await storage.getLabPosts(parseInt(req.params.id));
      
      res.json(posts);
    } catch (error) {
      console.error("[API] Error retrieving lab posts:", error);
      res.status(500).json({ message: "Failed to retrieve lab posts" });
    }
  });
  
  // Publish a lab post (creating a real post in the circle)
  app.post("/api/labs/:labId/posts/:postId/publish", async (req, res) => {
    console.log(`[API] POST /api/labs/${req.params.labId}/posts/${req.params.postId}/publish - Publishing lab post`);
    ...
    try {
      // Get the lab first to verify ownership
      const lab = await storage.getLab(parseInt(req.params.labId));
      ...
      // Verify ownership
      if (lab.userId !== req.user!.id) {
        return res.status(403).json({ message: "Not authorized to publish posts for this lab" });
      }
      
      // Publish the post
      const updatedPost = await storage.publishLabPost(
      ...
      console.error("[API] Error publishing lab post:", error);
      res.status(500).json({ message: "Failed to publish lab post" });
    }
  });
  
  // Get lab metrics
  app.get("/api/labs/:id/metrics", async (req, res) => {
    console.log(`[API] GET /api/labs/${req.params.id}/metrics - Retrieving lab metrics`);
    ...
    try {
      ...
      // Verify ownership
      if (lab.userId !== req.user!.id) {
        return res.status(403).json({ message: "Not authorized to view metrics for this lab" });
      }
      
      // Get lab metrics
      const metrics = await storage.getLabMetrics(parseInt(req.params.id));
      
      if (!metrics) {
        // If no metrics exist yet, calculate them
        const newMetrics = await storage.calculateLabMetrics(parseInt(req.params.id));
        res.json(newMetrics);
      } else {
        res.json(metrics);
      }
    } catch (error) {
      console.error("[API] Error retrieving lab metrics:", error);
      res.status(500).json({ message: "Failed to retrieve lab metrics" });
    }
  });
  
  // Calculate and update lab metrics
  app.post("/api/labs/:id/metrics/calculate", async (req, res) => {
    console.log(`[API] POST /api/labs/${req.params.id}/metrics/calculate - Calculating lab metrics`);
    ...
    try {
      ...
      // Verify ownership
      if (lab.userId !== req.user!.id) {
        return res.status(403).json({ message: "Not authorized to calculate metrics for this lab" });
      }
      
      // Calculate metrics
      const metrics = await storage.calculateLabMetrics(parseInt(req.params.id));
      
      res.json(metrics);
    } catch (error) {
      console.error("[API] Error calculating lab metrics:", error);
      res.status(500).json({ message: "Failed to calculate lab metrics" });
    }
  });
