...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/labs/:labId/circles/:circleId", async (req, res) => {
    ...
    try {
      ...
      // Update access level based on the new role
      const accessRole = role === "observation" ? "viewer" : "collaborator";
      
      try {
        console.log(`[API] Updating access to ${accessRole} for user ${req.user!.id} for circle ${circleId}`);
        await storage.grantCircleAccessToUser(req.user!.id, circleId, accessRole);
      } catch (accessError) {
        console.error("[API] Error updating circle access:", accessError);
        // Continue even if access granting fails, as the circle role was already updated
      }
